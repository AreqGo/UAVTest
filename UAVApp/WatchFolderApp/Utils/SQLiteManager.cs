...
namespace WatchFolderApp.Utils
{
    public static class SQLiteManager
    {
        public static string DBFileName = "D:\\db\\UAV.db";
        ...
        public static bool InsertData(string condition)
        {
            if (DBFileName != "")
            {
                SQLiteConnection conn = null;
                ...
                string sql = "insert into T_Design (TaskID,TaskName,TaskUAV,TaskTime) values ("+condition+")";
                SQLiteCommand cmdQ = new SQLiteCommand(sql, conn);
                int res = cmdQ.ExecuteNonQuery();
                conn.Close();
                if (res== -1)
                {
                    return true;
